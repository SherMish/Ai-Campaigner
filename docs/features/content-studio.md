...
The renderer uses the production palette (`#171717`, `#F7F2EA`, `#EDE6DA`, `#FF5A36`, `#2FA36B`, `#665CFF`), Rubik/IBM Plex Mono typography and `/favicon.png`. Opening slides are intentionally high-contrast and sparse; content slides hold one idea. Hook labels size themselves to their copy and center it vertically rather than occupying a generic wide pill. The signal question motif stays above the footer and shares one geometric center with its question mark. The checklist hook instead uses a sharp-corner orange save tag with a bookmark icon, so `שמרו את הפוסט` reads as an Instagram action rather than another generic CTA. The checklist's opening promise, three numbered tiles and three check slides are one locked sequence; its numbers and symbols sit on the right, center from the badge geometry, and share one right edge with the title, body and footnote. Its warning slide contains only the warning title and explanation inside the outlined frame, without a button-like accent competing with the final CTA. The signal metric slide presents the entered metric once without inserting a second orange unit label. CTA slides mention Ads Agent gently, use one text brand marker rather than a duplicate image logo, keep all dark copy outside the dark decorative shape, and finish with a content-sized free-start button whose arrow points left with the reading flow.
