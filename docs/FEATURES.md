...
| `shareOfCampaignSpend` | `(objectSpendAgorot, campaignSpendAgorot) → ratio \| null` | Null when the campaign spent nothing — "no share of nothing" isn't 0%. Consumer added AIC-85: `comparableCreatives`/`comparableAdsets` (`rules.ts`) use it as the dormancy check (`< 10%` share = not really running) — a relative, scale-free definition of "comparable" that's independent of the absolute `MIN_CREATIVE_SPEND_AGOROT`/`AUDIENCE_MIN_SPEND_AGOROT` evidence gates. See [RULES.md](RULES.md#comparability--the-add_creatives_for_comparison-advisory-aic-8586). |
