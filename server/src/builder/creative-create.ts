...
export type CreativeSpec =
  | { kind: "upload"; adAccountId: string; pageId: string; name: string; headline: string; primaryText: string; whatsappNumber: string; media: CreativeMedia }
...
export async function createCreativeIdempotent(
  ...
  const payload: Record<string, unknown> =
    spec.kind === "upload"
      ? { adAccountId: spec.adAccountId, pageId: spec.pageId, name: spec.name, headline: spec.headline, primaryText: spec.primaryText, whatsappNumber: spec.whatsappNumber, media: spec.media }
